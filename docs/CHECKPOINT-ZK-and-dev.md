...
## 2أ) تصحيح IP جهاز في قاعدة البيانات (بدون تغيير الراوتر)

من مجلد `backend` (مع `DEVICE_ID` الصحيح من جدول الأجهزة):

```bash
DEVICE_ID=2 DEVICE_IP=192.168.0.47 npm run device:set-ip
```

أو: `node scripts/set-device-lan-ip.js`
...
## 2ب) تشخيص سريع إذا «لا يتصل» رغم IP صحيح في النظام

من **نفس الجهاز الذي يشغّل Node (الـ API)**:

```bash
ping 192.168.0.47
```

إذا ping لا يصل، أو `TCP timeout` على المنفذ 4370، فالمشكلة **شبكة / جدار ناري / عنوان خاطئ / VLAN** وليست حقل `ip_address` في قاعدة البيانات وحده.
