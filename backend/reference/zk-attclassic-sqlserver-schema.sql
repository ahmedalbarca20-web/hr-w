/*
  ZKTeco / legacy attendance — Microsoft SQL Server DDL (classic CHECKINOUT layout).
  Copied for interoperability reference from vendor install:
    C:\Program Files (x86)\AttMid-east\sqlserver.sql
  This HR app does not run these tables; use them only when migrating data from
  old Windows clients (e.g. AttMid-east) or aligning field meanings with ZK logs.

  Typical mapping to this codebase:
    USERINFO.BADGENUMBER  -> employee card / device user id string
    CHECKINOUT.USERID     -> numeric user id on device (join USERINFO.USERID)
    CHECKINOUT.CHECKTIME  -> punch time (UTC/local per source DB)
    CHECKINOUT.CHECKTYPE  -> 'I' check-in, 'O' check-out (vendor convention)
    CHECKINOUT.VERIFYCODE -> ZK verification / state code (see zkteco docs / device logs)
*/
