...
async function probeSnapshot(opts) {
  ...
  try {
    ...
    if (code === 'ECONNREFUSED' || /ECONNREFUSED|refused/i.test(message)) {
      ...
      result.hint_ar =
        'انتهت مهلة الاتصال (TCP/UDP) على منفذ ZK (غالباً 4370): الحاسبة التي تُشغّل فيها عملية الاتصال (خادم الـ API أو — مع Local Agent — الحاسبة التي عليها برنامج الوكيل 8099) يجب أن ترى الجهاز على LAN. تحقق: (1) IP الجهاز من شاشته أو الراوتر، (2) نفس الشبكة/النطاق (مثلاً 192.168.0.x مع الجهاز)، (3) من تلك الحاسبة: ping ثم اختبار المنفذ (PowerShell: Test-NetConnection 192.168.x.x -Port 4370)، (4) عزل عملاء Wi‑Fi معطّل، (5) جدار Windows يسمح لـ Node بالخروج، (6) إن كان الـ API على سحابة استخدم جسر LAN أو وكيل على المكتب.';
