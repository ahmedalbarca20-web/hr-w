...
const { Employee, Department, WorkShift, User, Role, Company } = require('../models/index');
...
/** Domain for auto-created employee portal addresses (e.g. wfood1@hr.com). */
const portalEmailDomain = () => {
  const d = String(process.env.EMPLOYEE_PORTAL_EMAIL_DOMAIN || 'hr.com').trim().toLowerCase();
  return d.replace(/^@/, '') || 'hr.com';
};

/**
 * Alphanumeric slug from company login code (tax_id), else company name, else co{id}.
 * Lowercase so emails look like wfood1@hr.com.
 */
function companySlugForPortalEmail(company) {
  const raw = String(company?.tax_id || company?.name || `co${company?.id || ''}`).trim();
  const slug = raw.toLowerCase().replace(/[^a-z0-9]/g, '');
  return (slug || `co${company.id}`).slice(0, 28);
}

/**
 * Canonical login-style email: {companySlug}{employee_number}@domain
 * Example: company code wfood + fingerprint id 1 → wfood1@hr.com
 */
function buildEmployeePortalEmail(company, employee) {
  const slug = companySlugForPortalEmail(company);
  const num = String(employee?.employee_number ?? '')
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]/g, '');
  const domain = portalEmailDomain();
  if (!num) {
    const local = `${slug}e${employee.id}`.slice(0, 64);
    return `${local}@${domain}`;
  }
  const local = `${slug}${num}`.slice(0, 64);
  return `${local}@${domain}`;
}

/** True if this address was generated by an older version of the importer. */
function isLegacyPortalEmail(email) {
  const e = String(email || '').toLowerCase();
  return e.endsWith('@internal.hr') || /^emp\.\d+\.\d+@/.test(e);
}
 ...
 * User email in the directory: {companyCode}{employee_number}@hr.com (e.g. wfood1@hr.com).
 */
async function ensurePortalUserAfterZkImport(company_id, employee) {
  if (!employee?.id) return null;
  try {
    const company = await Company.findByPk(company_id, { attributes: ['id', 'tax_id', 'name'] });
    ...
    const desiredEmail = buildEmployeePortalEmail(company, employee).toLowerCase();

    const linked = await User.findOne({ where: { employee_id: employee.id, company_id } });
    if (linked) {
      const cur = String(linked.email || '').toLowerCase();
      if (cur !== desiredEmail && isLegacyPortalEmail(linked.email)) {
        const taken = await User.findOne({
          where: { email: desiredEmail, id: { [Op.ne]: linked.id } },
        });
        if (!taken) await linked.update({ email: desiredEmail });
      }
      return linked;
    }

    let email = desiredEmail;
    let busy = await User.findOne({ where: { email } });
    if (busy) {
      const slug = companySlugForPortalEmail(company);
      const num = String(employee.employee_number || '')
        .trim()
        ...
        .replace(/[^a-z0-9]/g, '');
      const domain = portalEmailDomain();
      email = `${slug}${num || 'n'}_${employee.id}@${domain}`.toLowerCase().slice(0, 90);
      busy = await User.findOne({ where: { email } });
      if (busy) {
        email = `u${company_id}_emp${employee.id}@${domain}`.toLowerCase();
      }
    }
